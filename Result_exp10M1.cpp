...
string MPFR(double input){
    ...
    mpfr_exp10(result, x, MPFR_RNDN);
    mpfr_sub(m, result, one, MPFR_RNDN);

    mpfr_sprintf(buffer, "%.18Rf", m);
    string num1(buffer);
...
string POSIT(double input){
    ...
    string num2;
    Posit64 ans = Posit_exp10Minus1(input);
    ostringstream out;
...
void Run(mt19937 generator, double *interval){
    ...
    ofs.open("output/Posit_exp10M1.csv");
