...
void Run(mt19937 generator, double *interval){
    uniform_int_distribution<int>  site(1, 9);
    mpfr_t result;
    int target;
    double input;
    string num1, num2, num3;
    vector<double> IEEE, POS;
    ofs.open("output/Posit_acosh.csv");
    ofs <<  "index,input,MPFR,Posit64,Double,winner" << "\n";

    for(int i = 0; i < 1000; i++){
        target = site(generator);                             
        uniform_real_distribution<double> range(1, 2);  
