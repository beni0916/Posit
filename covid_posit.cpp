...
#include <cstdlib>
#include <random>
// g++ -std=c++17 -o covid_posit covid_posit.cpp Posit_sqrt.cpp Posit_fabs.cpp
using namespace std;
// cnt = 9282
...
char train_file[30] = "covid_train.txt";
char test_file[30] = "covid_test.txt";
char sep_word = '\t';
int total_col = 21;
int target_col = 20;
...
int main(void)
{
    ...
    int cnt = 0;
    for(int i = 0; i < y_pred_test.size(); ++i)
    {
        Posit64 f = Posit_fabs(y_pred_test[i][0]);
        Posit64 b = Posit_fabs(y_pred_test[i][0] - Posit64{1});

        if(f < b)
            y_pred_test[i][0] = Posit64{0};
        else
            y_pred_test[i][0] = Posit64{1};

        if(y_pred_test[i][0] == y_test_orig[i])
            cnt++;
    }
    cout << "cnt = " << cnt << endl;
    cout << "persent = " << cnt / y_pred_test.size() * 100 << endl;
...
void load_data(string file_name, vector<vector<Posit64>> &store_arr, vector<Posit64> &target)
{
    ...
    int LEN = 10000;
	while(cnt < LEN && getline(train_file, line)) //cnt < LEN && 
	{
		cnt++;
