...
#include <iomanip>
#include "myfdlibm.h"
// g++ -std=c++17 -o linear_posit linear_posit.cpp Posit_fabs.cpp Posit_sqrt.cpp Posit_exp2.cpp Posit_floor.cpp Posit_log2.cpp Posit_pow.cpp
using namespace std;
int LEN = 2000;
char train_file[30] = "linear24_train.txt";
char test_file[30] = "linear24_test.txt";
char sep_word = '\t';
int total_col = 11;
int target_col = 10;
...
void frexp(vector<Posit64> &src, Posit64 &max, Posit64 &min);
...
void caculate(vector<vector<Posit64>> data, vector<Posit64> x, vector<Posit64> true_val, Posit64 &max, Posit64 &min)
{
	...
	{	
		...
		// prediction = prediction * Posit_pow(2, (max - min) / 2);
...
void frexp(vector<Posit64> &src, Posit64 &max, Posit64 &min)
{
    ...
    {
        Posit64 e = Posit_log2(src[i]);
        Posit64 v = src[i] / (Posit_pow(2, e));
        Posit64 e_0 = (t_max - t_min) / 2;
        src[i] = v * Posit_pow(2, e - e_0);
    }
}