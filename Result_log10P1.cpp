...
string MPFR(double input){
    ...
    mpfr_set_d(x, input + one, MPFR_RNDN); 
    mpfr_log10(result, x, MPFR_RNDN);
...
string POSIT(double input){
    string num2;
    Posit64 ans = Posit_log10Plus1(input);
    ostringstream out;
...
void Run(mt19937 generator, double *interval){
    ...
    ofs.open("output/Posit_log10P1.csv");
