...
string MPFR(double base, int input){
    mpfr_t x, y, result;
    double tem = 1. / (double)input;
    char buffer[50];
...
string POSIT(double base, int input){
    string num2;
    
    Posit64 ans = Posit_rootN(base, input);
    ostringstream out;
...
string IEEE754(double base, int input){
    string num3;
    double ans = pow(base, 1. / (double)input);
    ostringstream out;
...
void Run(mt19937 generator, double *interval){
    ...
    ofs.open("output/Posit_rootN.csv");
    ofs <<  "index,base,input,MPFR,Posit64,Double,winner" << "\n";
    ...
    for(int i = 0; i < 1000; i++){
        ...
        ofs << i << "," << base << "," << input << "," << num1 << "," << num2 << "," << num3 << ",";
