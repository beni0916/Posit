...
#ifdef __STDC__
	Posit64 Posit_atanPi(Posit64 x)
#else
	Posit64 atanPi(x)
	Posit64 x;
#endif
{
	...
	if(fabs_x >= limit1) {	/* if |x| >= 2^66 */
	    ...
	    if(hx>0) return  (atanhi[3]+atanlo[3]) / PI;
	    else     return (-atanhi[3]-atanlo[3]) / PI;
	} 
    if (fabs_x < limit2) {	/* |x| < 0.4375 */
	    if (fabs_x < limit3) {	/* |x| < 2^-29 */
		    if(huge+x>one) return x / PI;	/* raise inexact */
	    }
	    id = -1;
	} 
    else {
	...
	if (id<0) return (x - x*(s1+s2)) / PI;
	else {
	    z = atanhi[id] - ((x*(s1+s2) - atanlo[id]) - x);
	    return (hx<0)? -z / PI : z / PI;
	}
}
