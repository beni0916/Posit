...
// g++ -std=c++17 -o msd_posit msd_posit.cpp Posit_fabs.cpp Posit_sqrt.cpp Posit_exp2.cpp Posit_floor.cpp

int main(void)
{
	vector<vector<Posit64>> train_A;
	vector<Posit64> train_b;
	load_data("MSD_train.txt", train_A, train_b);
	
	vector<vector<Posit64>> test_A;
	vector<Posit64> test_b;
	LEN = 1000;
	load_data("MSD_test.txt", test_A, test_b);
