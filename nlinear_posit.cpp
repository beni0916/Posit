...
#include <iomanip>
#include "myfdlibm.h"
// g++ -std=c++17 -o nlinear_posit nlinear_posit.cpp Posit_fabs.cpp Posit_sqrt.cpp Posit_exp2.cpp Posit_floor.cpp Posit_log2.cpp Posit_pow.cpp
using namespace std;
int LEN = 2000;
...
void load_data(string file_name, vector<vector<Posit64>> &store_arr, vector<Posit64> &target)
{
	...
	while(getline(train_file, line)) //cnt < LEN && 
	{
		...
		while(getline(ss, token, sep_word))
		{
			...
			value = value / Posit64{100} + Posit64{1};
