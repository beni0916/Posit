...
#include <iomanip>
#include "myfdlibm.h"
// g++ -std=c++17 -o collinear_posit collinear_posit.cpp Posit_fabs.cpp Posit_sqrt.cpp Posit_exp2.cpp Posit_floor.cpp
using namespace std;
int LEN = 2500;
char train_file[30] = "collinear_high_p_train.txt";
char test_file[30] = "collinear_high_p_test.txt";
char sep_word = '\t';
int total_col = 7;
int target_col = 6;
	...
	while(getline(train_file, line)) //cnt < LEN && 
	{
		...
		while(getline(ss, token, sep_word))
		{
			...
			if(!token.empty())
			{
				try
				{
					value = Posit64{stod(token)}  + Posit64{0.000001} ; //  + Posit64{0.000001} 
				}
				catch(...)
				{
					value = Posit64{0.0};
