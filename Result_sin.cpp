...
void Run(mt19937 generator, double *interval){
    ...
    ofs.open("output/Posit_sin.csv");
    ofs <<  "index,input,MPFR,Posit64,Double,winner" << "\n";

    for(int i = 0; i < 1000; i++){
        target = site(generator);                             
        uniform_real_distribution<double> range(0, 1);   // interval[target], interval[target + 1] 3.14, 4.71 4.72, 6.28  1.58, 3.14
