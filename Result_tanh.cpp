...
void RMSE(vector <double> &ieee, vector<double> &pos){
    ...
    ofs.close();
    cout << "Posit: " << sqrtl((sum2/count)) << endl;
    cout << "IEEE:  " << sqrtl((sum1/count)) << endl;
}
...
void Run(mt19937 generator, double *interval){
    ...
    ofs.open("output/Posit_tanh.csv");
    ofs <<  "index,input,MPFR,Posit64,Double,winner" << "\n";

    for(int i = 0; i < 1000; i++){
        target = site(generator);                             
        uniform_real_distribution<double> range(-3, 3);  
