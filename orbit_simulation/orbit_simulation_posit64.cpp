...
const Posit64 G_AU = Posit64(39.478440530965371); // 萬有引力常數 (單位：AU^3 yr^-2 M_sun^-1)
const Posit64 SOLAR_MASS = Posit64(1.0); // 太陽質量 (單位：M_sun)
...
Vector2D calculateAcceleration(const Vector2D& pos) {
    ...
    Posit64 distance = Posit_sqrt(pos.x * pos.x + pos.y * pos.y);
    Posit64 accMagnitude = (G_AU * SOLAR_MASS) / (distance * distance);
...
// RK4 數值積分方法
void runSimulation_RK4(Posit64 dt, int steps, const char* filename) {
    Vector2D pos = {1.0, 0.0}; // 初始位置 (1 AU, x軸)
    Posit64 iniV = Posit_sqrt(G_AU);
...
// 主程式
int main() {
    const Posit64 TIME_STEP_AU = 0.001; // 時間步長 (單位：yr)
    const int SIM_STEPS = 100000; // 模擬步數 (約10年)

    // 執行 RK4 模擬
    runSimulation_RK4(TIME_STEP_AU, SIM_STEPS, "Posit64.csv");
