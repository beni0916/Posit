...
#include <iomanip>
#include "myfdlibm.h"
// g++ -std=c++17 -o test_posit test_posit.cpp Posit_fabs.cpp Posit_sqrt.cpp Posit_exp2.cpp Posit_floor.cpp
using namespace std;
int LEN = 2500;
#define arr_len 30
char train_arr[arr_len][30] = {"AME", "cal", "auto", "concrete", "HPD", "HPI", "insurance", "linear", "linear1", "linear2", "linear3", "linear4", "linear5", "linear6", "linear7", "linear8", "linear9", "hetero", "boundary", "collinear", "collinear_extreme", "collinear_high_p", "collinear_high", "collinear_low_p", "collinear_low", "collinear_mid_p", "collinear_mid", "CP", "btsp", "melb"};
char sep_arr[arr_len + 1] = "\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t";
int total_arr[arr_len] = {37, 9, 8, 9, 13, 22, 4, 10, 10, 20, 20, 20, 30, 30, 32, 33, 23, 28, 13, 11, 11, 7, 11, 7, 11, 7, 11, 15, 6, 13};
int target_arr[arr_len] = {36, 8, 0, 8, 0, 21, 3, 9, 9, 19, 19, 19, 29, 29, 31, 32, 22, 27, 12, 10, 10, 6, 10, 6, 10, 6, 10, 14, 5, 1};
...
void d1_normalization(vector<Posit64> &src, Posit64 &nor_max, Posit64 &nor_min)
{
	...
	for(int i = 0; i < len; i++)
	{
		...
		// src[i] = Posit64{0.00390625} + (src[i] - min) / diff * Posit64{127.99609375};
		// src[i] = Posit64{0.0000152587890625} + (src[i] - min) / diff * Posit64{32767.9999847412109375};
	}
}
...
vector<Posit64> gaussian_elimination(vector<vector<Posit64>> &A, vector<Posit64> &b)
{
	...
	for(int i = 0; i < n; i++)
	{
		...
		for(int k = i + 1; k < n; k++)
		{
			if(Posit_fabs(A[k][i]) > Posit_fabs(A[max_row][i]))
				max_row = k;
		}
		swap(A[i], A[max_row]);
		swap(b[i], b[max_row]);
...
void caculate(vector<vector<Posit64>> data, vector<Posit64> x, vector<Posit64> true_val, Posit64 &max, Posit64 &min)
{
	...
	{	
		...
		// prediction = (prediction - Posit64{0.00390625}) / Posit64{127.99609375} * diff + min;
		// prediction = (prediction - Posit64{0.0000152587890625}) / Posit64{32767.9999847412109375} * diff + min;
