...
// g++ -std=c++17 -o wine_posit wine_posit.cpp Posit_fabs.cpp Posit_sqrt.cpp Posit_exp2.cpp Posit_floor.cpp
...
void caculate(vector<vector<Posit64>> data, vector<Posit64> x, vector<Posit64> true_val, Posit64 &max, Posit64 &min)
{
	...
	{	
		...
		cout << "i = " << i << " prediction = " << prediction  << " true_val = " << true_val[i] << endl;
	...
	Posit64 sl = squared_error_sum / len;
	Posit64 rmse = Posit_sqrt(sl);
	Posit64 mae = abs_error_sum / len;
	Posit64 y_mean = y_sum / len;
	Posit64 rmse_percent = (rmse / y_mean) * Posit64{100.0};
	Posit64 mae_percent = (mae / y_mean) * Posit64{100.0};


	//double tmp = sw::universal::convert_to_double(squared_error_sum / len);
	//double tmp_p = (tmp / (static_cast<double>(y_mean))) * 100;

	cout << "LEN: " << LEN << endl;
	cout << "abs_error_sum: " << abs_error_sum << endl;
	cout << "y_mean: " << y_mean << endl;
	cout << "Posit32" << endl;
	cout << "RMSE: " << setprecision(20) << rmse << " (" << rmse_percent << "% of mean true value)" << endl;
	cout << "MAE: " << setprecision(20) << mae   << " (" << mae_percent << "%"<< endl;
}
