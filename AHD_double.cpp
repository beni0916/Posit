...
int LEN = 100;
void d1_normalization(vector<double> &, double &, double &);
...
void caculate(vector<vector<double>> data, vector<double> x, vector<double> true_val, double &, double &);

// g++ -std=c++17 -o AHD_double AHD_double.cpp

int main(void)
{
	vector<vector<double>> train_A;
	vector<double> train_b;
	load_data("AHD_train.txt", train_A, train_b);
	//cout << "size = " << train_A.size() << endl;

	vector<vector<double>> test_A;
	vector<double> test_b;
	load_data("AHD_test.txt", test_A, test_b);
...
void caculate(vector<vector<double>> data, vector<double> x, vector<double> true_val, double &nor_max, double &nor_min)
{
	int len = data.size();
	double squared_error_sum = 0.0;
	double abs_error_sum = 0.0;
	double y_sum = 0.0;
	double diff = nor_max - nor_min;
	if(diff == 0)
        diff = 1e-6;
	...
	{	
		...
		prediction = prediction * diff + nor_min;
