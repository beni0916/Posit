...
#ifdef __STDC__
	Posit64 Posit_expMinus1(Posit64 x)	/* default IEEE double exp */
#else
	Posit64 Posit_expMinus1(x)	/* default IEEE double exp */
	Posit64 x;
#endif
{
	Posit64 y, hi, lo, c, t;
	...
	if(x>1) return exp(x-1) * exp(1) - 1;
	else if(x<-1) return exp(x+1) / exp(1) - 1;
